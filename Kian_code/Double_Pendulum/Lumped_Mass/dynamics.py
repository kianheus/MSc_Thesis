...
def dynamical_matrices(rp: dict, q: Tensor, q_d: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
    ...
    c1 = torch.cos(q[0, 0]).unsqueeze(0)
    c2 = torch.cos(q[0, 1]).unsqueeze(0)
    s12 = torch.sin(q[0, 0]-q[0, 1]).unsqueeze(0)
    c12 = torch.cos(q[0, 0]-q[0, 1]).unsqueeze(0)
    ...
    C_q_1 = torch.cat((C_q_11, (rp["l1"] * rp["l2"] * rp["m2"] * q_d[0, 1] * s12).to(device)), dim=0).unsqueeze(0)
    C_q_2 = torch.cat(((-rp["l1"] * rp["l2"] * rp["m2"] * q_d[0, 0] * s12).to(device), C_q_11), dim=0).unsqueeze(0)
...
def input_matrix(rp: dict, q: Tensor) -> Tensor:
    ...
    Rx = rp["xa"] - rp["l1"] * torch.cos(q[0, 0]) - rp["l2"] * torch.cos(q[0, 1])
    Ry = rp["ya"] - rp["l1"] * torch.sin(q[0, 0]) - rp["l2"] * torch.sin(q[0, 1])

    A_q1 = ( (rp["l1"] * torch.sin(q[0, 0]) * Rx - rp["l1"] * torch.cos(q[0, 0]) * Ry) / torch.sqrt(Rx**2 + Ry**2) ).unsqueeze(0).unsqueeze(0)
    A_q2 = ( (rp["l2"] * torch.sin(q[0, 1]) * Rx - rp["l2"] * torch.cos(q[0, 1]) * Ry) / torch.sqrt(Rx**2 + Ry**2) ).unsqueeze(0).unsqueeze(0)
