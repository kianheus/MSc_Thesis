...
class Autoencoder_double(nn.Module):
    ...
    def encoder_theta_0_ana(self, q):
        theta_0 = transforms_SEA.analytic_theta_0(self.rp, q).unsqueeze(0)
    ...
    def encoder_theta_1_ana(self, q):
        theta_1 = transforms_SEA.analytic_theta_1(self.rp, q).unsqueeze(0)
        return theta_1, theta_1
    
    def theta_ana(self, q):
        theta_ana = torch.vmap(transforms_SEA.analytic_theta, in_dims=(None, 0))(self.rp, q)
        return theta_ana
...
class Analytic_transformer():
    ...
    def encoder(self, q):
        theta_ana = transforms_SEA.analytic_theta(self.rp, q)
        return theta_ana
    
    def decoder(self, theta, clockwise):
        q_ccw, q_cw = torch.vmap(transforms_SEA.analytic_inverse, in_dims=(None, 0))(self.rp, theta)
        if clockwise:
            return q_cw
        else:
            return q_ccw
    ...
    def decoder_cw(self, theta):
        q_ccw, q_cw = transforms_SEA.analytic_inverse(self.rp, theta)
        return q_cw
